...
export const useRoomRealtimeReadyStore = create<RealtimeStore>(
  (set, get) => ({
    channel: null,
    ...
    sendReady: async (userId: string) => {
      ...
      if (!channel) {
        console.error("❌ Channel not found - Please wait for subscription to complete");
        return false;
      }
